...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    hinge_loss = total_loss/len(labels)
...
def perceptron_single_step_update(
    ...
    """
    # Your code here
    prediction = label * (np.dot(feature_vector,current_theta)+current_theta_0)
    if prediction<=0 or abs(prediction) <= 0.0000005:
...
def average_perceptron(feature_matrix, labels, T):
    ...
    # Your code here
    ave_theta = total_theta/(T * feature_matrix.shape[0])
    ave_theta_0 = total_theta_0/(T * feature_matrix.shape[0])
...
def classifier_accuracy(
    ...
    train_classify = classify(train_feature_matrix, theta, theta_0)
    val_classify = classify(val_feature_matrix, theta, theta_0)

    train_acc = accuracy(train_classify, train_labels)
    val_acc = accuracy(val_classify, val_labels)
